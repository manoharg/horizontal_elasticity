...
#function to scale the containers up and down 
def ScaleContaienr(meanCPU, end_Cool_time, containerCount, containerName):
    ...
    if (end_Cool_time and (curr_time < end_Cool_time)):
        ...
        if (meanCPU > 5):
            ...
            compose_scale_call(compose_cmd) #scaling up containers
            end_Cool_time = curr_time + datetime.timedelta(seconds=coolPeriod)
        elif (meanCPU < 5 and containerCount > 1):
            containerCount -= 1
            end_Cool_time = curr_time + datetime.timedelta(seconds=coolPeriod)
            print('Scaling down containers to %s' % containerCount)  #scaling down containers
            compose_cmd  = "docker-compose scale " + containerName + "=" + str(containerCount)
...
def compose_scale_call(compose_cmd):
    os.chdir('/home/manohar/Desktop/script/compose')
    os.system(compose_cmd)

#function to calculate mean cpu
def handleCPUSeries(cpuseries, cpu):
    ...
    logging.debug(cpuseries)  #debugging
...
def getContainerInComposeMode(con_name_pattern, env):
    ...
    regexptn = "^" + con_name_pattern + ".*"; #to match with the service name
